...
/**
 * `FiberRuntime.unwindSuccess` and `FiberRuntime.evalSync` invoke application
 * lambdas on behalf of `runLoop`. They must stay above C2's `FreqInlineSize`
 * (325 bytes of bytecode) so C2 never inlines them, and with them the
 * application's lambdas, into `runLoop` (zio/zio#11251). This spec reads the
 * class file and fails if a refactor drops either helper below the threshold.
 */
object FiberRuntimeHelperSizeSpec extends ZIOBaseSpec {
  ...
  def spec = suite("FiberRuntimeHelperSizeSpec")(
    test("run loop helpers are too large for C2 to inline into runLoop") {
      ...
      assertTrue(size("unwindSuccess") > FreqInlineSize, size("evalSync") > FreqInlineSize)
    }
  )
}
