...
/**
 * `FiberRuntime.coldStep` holds the cold cases of `runLoop`. It must stay above
 * C2's `FreqInlineSize` (325 bytes of bytecode) so C2 never inlines it, and
 * with it the cold cases, into `runLoop` (zio/zio#11251). This spec reads the
 * class file and fails if a refactor drops the helper below the threshold.
 */
object FiberRuntimeHelperSizeSpec extends ZIOBaseSpec {
  ...
  def spec = suite("FiberRuntimeHelperSizeSpec")(
    test("run loop helpers are too large for C2 to inline into runLoop") {
      ...
      assertTrue(size("coldStep") > FreqInlineSize)
    }
  )
}
